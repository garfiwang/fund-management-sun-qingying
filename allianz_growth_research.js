/**
 * Allianz Income and Growth Multi Asset Fund - Research Page Script
 */
...
async function loadFundDetails() {
  try {
    const res = await fetch('data/allianz_details.json');
    if (res.ok) {
      fundData = await res.json();
    }
  } catch (err) {
    console.error('Failed to load allianz_details.json');
  }
}

function renderQuickMetrics() {
  if (!fundData) return;
  const latestNav = fundData.latest_nav || 8.6000;
...
function renderHoldingsTable() {
  ...
  fundData.top_holdings.forEach(item => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-weight:700; color:var(--color-emerald);">${item.rank}</td>
      <td style="font-weight:700;">${item.name}</td>
      <td><span class="badge-tx dividend" style="font-size:0.75rem;">${item.type}</span></td>
    ...
    fundData.regions.forEach(item => {
      ...
      div.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.88rem;">
          ...
          <span style="font-weight:800; color:var(--color-emerald);">${item.weight}</span>
        </div>
        <div class="alloc-bar-bg">
          <div class="alloc-bar-fill" style="width: ${pct}%; background: var(--color-emerald);"></div>
        </div>
...
function renderYearlySummary() {
  ...
  fundData.yearly_nav_summary.forEach(item => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-weight:800; color:var(--color-emerald);">${item.year}</td>
