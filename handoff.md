...
5. **雙帳戶總儀錶板架構重構**：
   ...
   - 建立獨立子頁面 `featured_funds.html`：展示精選 7 檔基金標的獨立研究報告與 30 天淨值監控。
   - 建立獨立子頁面 `china_property_soe_research.html`：展示 4 檔中國地產國企 (SOEs) 新基金投資紀錄與對比矩陣。
   - 統一導覽列按鈕，支援在所有頁面間快速切換。

## 🚦 目前狀態
- 客戶 sun 的 `accounts/sun_portfolio.md` 已補齊保單詳細資訊、估值、配息與扣款費用
- 所有基金 JSON 資料集已與網頁全數連動
...
- 最新進度：已成功重構儀錶板，建立兩大獨立子分頁 `featured_funds.html` 與 `china_property_soe_research.html`，主儀錶板恢復極致簡潔，GitHub Pages 已發布。
