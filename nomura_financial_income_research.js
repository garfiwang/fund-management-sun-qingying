/**
 * Nomura Global Financial Income Fund - Research Page Script
 */
...
async function loadFundDetails() {
  try {
    const res = await fetch('data/nomura_fin_details.json');
    if (res.ok) {
      fundData = await res.json();
    }
  } catch (err) {
    console.error('Failed to load nomura_fin_details.json');
  }
}

function renderQuickMetrics() {
  if (!fundData) return;
  const latestNav = fundData.latest_nav || 10.5850;
  const subPrice = fundData.subscription_price || 10.5000;
...
function renderHoldingsTable() {
  ...
  fundData.top_holdings.forEach(item => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-weight:700; color:#0d9488;">${item.rank}</td>
    ...
    fundData.regions.forEach(item => {
      ...
      div.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.88rem;">
          ...
          <span style="font-weight:800; color:#0d9488;">${item.weight}</span>
        </div>
        <div class="alloc-bar-bg">
          <div class="alloc-bar-fill" style="width: ${pct}%; background: #0d9488;"></div>
        </div>
...
function renderYearlySummary() {
  ...
  fundData.yearly_nav_summary.forEach(item => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-weight:800; color:#0d9488;">${item.year}</td>
