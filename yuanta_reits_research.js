/**
 * Yuanta Global Real Estate Fund - Research Page Script
 */
...
async function loadFundDetails() {
  try {
    const res = await fetch('data/yuanta_reits_details.json');
    if (res.ok) {
      fundData = await res.json();
    }
  } catch (err) {
    console.error('Failed to load yuanta_reits_details.json');
  }
}

function renderQuickMetrics() {
  if (!fundData) return;
  const latestNav = fundData.latest_nav || 11.7400;
  const subPrice = fundData.subscription_price || 11.8000;
...
function renderHoldingsTable() {
  ...
  fundData.top_holdings.forEach(item => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-weight:700; color:#ea580c;">${item.rank}</td>
      <td style="font-weight:700;">${item.name}</td>
      <td><span class="badge-tx sell" style="font-size:0.75rem;">${item.type}</span></td>
    ...
    fundData.regions.forEach(item => {
      ...
      div.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.88rem;">
          ...
          <span style="font-weight:800; color:#ea580c;">${item.weight}</span>
        </div>
        <div class="alloc-bar-bg">
          <div class="alloc-bar-fill" style="width: ${pct}%; background: #ea580c;"></div>
        </div>
...
function renderYearlySummary() {
  ...
  fundData.yearly_nav_summary.forEach(item => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-weight:800; color:#ea580c;">${item.year}</td>
